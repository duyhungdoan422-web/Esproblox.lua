...
local function toggleESP()
    ...
    for _, data in pairs(espList) do
        ...
        if data.tracer then data.tracer.Visible = espEnabled end
    end
end
...
local counterLabel = createCounter()
...
RunService.RenderStepped:Connect(function()
    ...
    if counterLabel then counterLabel.Text = "ESP: " .. count .. " players" end
end)
...
task.spawn(function()
    while true do
        task.wait(2)
        for player, data in pairs(espList) do
            if not Players:FindFirstChild(player.Name) or not data.character or not data.character.Parent then
                removeESP(player)
            end
        end
    end
