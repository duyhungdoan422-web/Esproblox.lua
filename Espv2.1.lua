...
RunService.RenderStepped:Connect(function()
    ...
    for player, data in pairs(espList) do
        if data.root and data.root.Parent then
            ...
            if onScreen and headPos.Z > 0 then
                ...
                data.tracer.From = Vector2.new(headPos.X, headPos.Y - height/4)
                data.tracer.To = topCenter
    ...
    -- Hiển thị số lượng trên màn hình (góc trên trái)
    local countGui = localPlayer.PlayerGui and localPlayer.PlayerGui:FindFirstChild("ESPCounter")
    if countGui then
        countGui.Text = "ESP: " .. count .. " players"
    end
end)

-- Tạo counter hiển thị số lượng
...
if localPlayer.PlayerGui and not localPlayer.PlayerGui:FindFirstChild("ESPCounter") then
    createCounter()
end
